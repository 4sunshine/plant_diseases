...
with torch.no_grad():
    model = FeatureOperator(61).cuda()
    model.eval()

    with tqdm(total=len(train_loader)) as pbar:
        for i, data in enumerate(train_loader):
            # output = model(data['images'])
            state['features'] += [model(data['features'].cuda()).cpu()]
            state['labels'] += data['labels']
            state['paths'] += data['paths']
            pbar.update(1)

        state['features'] = torch.cat(state['features'], dim=0)
        torch.save(state, os.path.join(root_dir, 'train_pooled.pth'))
    ...
    with tqdm(total=len(test_loader)) as pbar:
        ...
        state['features'] = torch.cat(state['features'], dim=0)
        torch.save(state, os.path.join(root_dir, 'test_pooled.pth'))
